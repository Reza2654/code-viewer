...
Write-Host "   Code Viewer - Build & Package Release         " -ForegroundColor Cyan
Write-Host "=================================================" -ForegroundColor Cyan
...
if (-not (Test-Path $distDir)) {
    ...
    Get-ChildItem -Path $distDir -Exclude "*.exe","*.zip" | Remove-Item -Recurse -Force
}

# 2. Publish Main CodeViewer Application
Write-Host "[1/5] Publishing CodeViewer (Release)..." -ForegroundColor Yellow
...
$zipPath = "$distDir\CodeViewer-v1.0.1-beta.1-win-x64-portable.zip"
if (Test-Path $zipPath) { Remove-Item $zipPath -Force }

# Gather files for ZIP (exclude installers)
$zipFiles = Get-ChildItem -Path $distDir -Exclude "*.zip","*.exe" | Select-Object -ExpandProperty FullName
...
if ($isccPath) {
    ...
    Write-Host "  -> Installer created: $distDir\CodeViewer-v1.0.1-beta.1-Setup.exe" -ForegroundColor Green
} else {
...
$hashEntries = @()
Get-ChildItem -Path "$distDir\*" -Include "*.exe","*.zip" -File | ForEach-Object {
